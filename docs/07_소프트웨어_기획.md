...
sequenceDiagram
    ...
    alt disposition = CAPTURE_AND_RETURN
        MM->>PS: 윈치 권취(상하)로 벨리 근처 안정화 → 그물째 운반
...
```
IDLE → ARMED → PATROL → TRACK → INTERCEPT → CINCH(입구 자동 조임)
                                  ├─ CAPTURE_RETURN → RECOVER(윈치 권취) → RTH
...
class MissionCommand(Protocol):
    ...
    def cinch_net(self) -> Ack: ...                  # 입구 조임 모터(자동, 인터록 필요)
    def recover_winch(self) -> Ack: ...              # 상하 권취(시닝 결속 완료가 게이트)
