...
> ⚠️ 이 문서는 **기획 + 파이프라인/모듈 아키텍처/인터페이스 정의**다.
> 실제 구현 코드는 다음 단계. **검증된 오픈소스 파서만** 쓴다 — 뇌피셜 파싱 금지.
...
## 6. 모듈 아키텍처 (`forensics/`) — 인터페이스 (실코드는 다음 단계)
...
├── pipeline.py          # 7단계 오케스트레이션 스켈레톤 (인터록/순서 강제)
└── chain_of_custody.py  # CoC 레코드 + append-only 로그 인터페이스
```
...
> 인터페이스/데이터클래스 스텁은 [`forensics/`](../forensics/) 참조. **실제 파싱·이미징 로직은
> 다음 단계**(요청 시 검증 OSS 어댑터 구현).
...
## 8. 다음 단계
- [ ] `forensics/interfaces.py` 데이터클래스/Protocol 확정 → 검증 OSS 어댑터 구현(pymavlink/pyulog/TSK).
- [ ] `chain_of_custody.py` append-only 로그 + 서명 검증 구현.
- [ ] 항적 → folium 리포트 템플릿(HTML/PDF) 프로토타입.
- [ ] 샘플 ArduPilot `.bin` / PX4 `.ulg` 골든 케이스로 파서 회귀테스트(`tests/`).
