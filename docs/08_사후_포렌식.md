...
- **★ 적 발사지점(HOSTILE LAUNCH SITE):** 홈/오리진 포인트 / 첫 측위 / 항적 역추적 →
  **발사·이륙 좌표** + **오차반경(CEP) + 신뢰도**. 이게 **반격 표적의 핵심 좌표.**
  (구현: `forensics/adapters/trajectory.py`의 `launch_estimate` + `launch_radius_m`.)
- **의도 표적(아군 방어 자산):** 웨이포인트·체공(loiter)·하강 패턴 → 적이 **노린 대상** 추정.
- **패턴 오브 라이프:** 발사지점을 **과거 포획 건과 교차** → 반복 출발지(=적 기지/발사 진지) 식별.
