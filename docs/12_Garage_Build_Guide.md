...
> `✅ CHECKPOINT P0`: BOM 77개 라인 전부 ✔ 또는 "발주됨". 손상품 0.
...
| 💨 CO2 솔레노이드/카트리지 (CRADLE 16g · TRAWLER 33g) | 고압. **정면 금지**. 발사 테스트는 **실외, 사람·창문·강아지 없는 방향**. 첫 테스트는 **그물 빼고 dry-fire**. TRAWLER는 대형·표준오프라 **사거리·후폭풍 더 큼 → 이격 더 크게**. |
| 🔥 cord-cutter 니크롬 소선 (TRAWLER) | 통전 시 **순간 적열(수백 ℃)** → 화상·발화. 벤치 테스트는 **가연물 치우고 짧은 펄스**로. 손가락·테더 외 물체 근처 금지. 솔레노이드 블레이드형은 협착 주의. |
...
4. **🟦 페이로드 소물**: Savox ×3, Pololu G2 ×2, INA226, HX711, CO2 솔레노이드 ×2(CRADLE 16g +
   **TRAWLER 대구경 33g**), IRLB3034 ×N, **니크롬 24AWG + 세라믹 홀더(cord-cutter)**, G5LE 릴레이.
...
> **AIM-PAN=SERVO9, AIM-TILT=SERVO10, CINCH=SERVO11, RELEASE=SERVO12,
> CRADLE FIRE=RELAY0, TRAWLER FIRE=RELAY1, CORD-CUTTER=RELAY2**.
> (ArduPilot는 1-인덱스라 payload_map RELAY0/1/2 = ArduPilot **RELAY1/2/3**.)
>
> **이중 그물:** 한 조준 짐벌(SERVO9/10)에 두 탄종이 올라탄다 — 🕊️**CRADLE**(정찰 생포: 소형
> 그물 RELAY0 → cinch SERVO11 → 윈치 SERVO12)와 💥**TRAWLER**(자폭 폐기: 대형 그물 RELAY1
> 표준오프 발사 → cord-cutter RELAY2로 테더 절단 투기). TRAWLER는 cinch·윈치 안 씀.

### 6.1 3D 출력 (BOM MAKE 부품)
- 출력물: PG-GIMBAL(요크/베어링), PG-SPOOL(시닝 스풀), PG-MUZZLE(CRADLE 머즐/베이),
  **PG-TRAWL-MUZZLE(TRAWLER 대형 머즐/캐니스터)**, 포렌식 케이스.
...
### 6.5 TRAWLER 표준오프 런처(RELAY1) + cord-cutter(RELAY2)
> 자폭 드론 전용. **접근 안 함** — 멀리서 큰 그물 던져 엉키게 하고, 테더 끊어 떨군다.

1. **대형 런처**: PG-TRAWL-MUZZLE에 **대구경 CO2 솔레노이드(33g 카트리지, DR-NP-020)** 장착.
   CRADLE 머즐 **옆(+Y 오프셋)** 에 같은 짐벌판에 볼트 — 한 터렛, 두 발사구. CO2 라인은
   CRADLE과 **독립 카트리지**(동시 장전).
2. **발사 드라이브(RELAY1)**: 솔레노이드(−) → **IRLB3034 로우사이드 MOSFET**(DR-NP-021) →
   **아밍 릴레이 하류** → 플라이백 1N5408. 코일은 ArduPilot **RELAY2** 핀.
3. **cord-cutter(RELAY2)**: **니크롬 24AWG 소선 + 세라믹 홀더**(DR-NP-022)를 **TRAWLER 테더가
   지나는 머즐 립**에 고정(소선이 테더를 감싸듯). 드라이브: **MOSFET + 아밍 릴레이 + 플라이백**
   (DR-NP-023) → ArduPilot **RELAY3** 핀.
   ```
   12V ─[아밍 릴레이 NO]─┬─ 니크롬 소선(테더 감김) ─┐
                         └──|◄ 1N5408 ────────────┘
   소선(-) ── IRLB3034 D | G←FC(RELAY3), S←GND   ; 짧은 펄스만! (적열→화상/발화)
   ```
   ⚠️ **펄스 길이 제한**: 니크롬은 통전 1~2초면 테더 끊김. 게이트 ON 유지하면 발화 → 펌웨어/스크립트로
   **타임아웃(예 ≤2s) 강제**. 솔레노이드 블레이드형이면 협착 주의.

### 6.6 TRAWLER 대형 그물 제작 + 패킹
1. **매듭없는 Dyneema/Kevlar 고밀도 메시**(셀 ~3cm로 더 촘촘 — 로터 확실히 결속),
   **마우스 ~6.5m**, `trawler_net()` 형상.
2. **중량 림 추**: 둘레 **8개+(브라스, CRADLE보다 무겁게)** — 광역 전개 운동량 확보.
3. **단일 중(重)테더**: 머즐 cord-cutter를 **반드시 통과**하도록 라우팅(여기가 끊기는 지점).
   퍼스 드로스트링·스풀 **없음**(TRAWLER는 cinch 안 함).
4. **패킹**: 더 크므로 꼬임에 더 민감 — 낙하산식 지그재그, 추 대칭 엄수.

> `✅ CHECKPOINT P6`: 짐벌 수동 팬/틸트 풀스윙(간섭 0). **CRADLE 트리거(RELAY1) + TRAWLER
> 트리거(RELAY2) + cord-cutter(RELAY3)** 전부 dry(그물·CO2 없이) 릴레이 딸깍. 니크롬 **짧은 펄스**로
> 적열 확인(가연물 치우고). 시닝/윈치 양방향 회전. INA226/HX711 값 읽힘.
...
# 그물 발사/절단 릴레이 (payload_map RELAY0/1/2 == ArduPilot RELAY1/2/3)
RELAY1_FUNCTION = 1      ; CRADLE FIRE   (payload_map RELAY0)
RELAY1_PIN = 50          ; AUX 출력 핀 번호(보드/펌웨어 버전에 맞게 확인)
RELAY2_FUNCTION = 1      ; TRAWLER FIRE  (payload_map RELAY1)
RELAY2_PIN = 51
RELAY3_FUNCTION = 1      ; CORD-CUTTER   (payload_map RELAY2) — Lua로 ON 타임아웃≤2s 강제
RELAY3_PIN = 52
# 수동 조종 / 페일세이프
...
> 드롭다운/`BRD_PWM_*`로 실제 사용 가능한 핀 확인 후 입력. payload_map 채널(9/10/11/12)·릴레이(RELAY1/2/3) **숫자**만 고정.
> cord-cutter(RELAY3)는 적열 발화 방지를 위해 **반드시 ON 타임아웃**(Lua 또는 페이로드 IF MCU, ≤2s).
...
`M`으로 그물 전환(레티클 초록 CRADLE↔빨강 대형 TRAWLER, 상태바 `NET ...`), TRAWLER서 `SPACE`=`FIRE_TRAWLER`, `X`=`CORD_CUT`.
...
| 4b | **모드 인터록** | `G`→`X`(CRADLE)→`M`→`C`(TRAWLER) | CRADLE서 cord-cut `DENIED`, TRAWLER서 cinch `DENIED` |
| 5 | E-Stop | `B` | 즉시 DISARM, throttle 0, 릴레이 OPEN |
| 6 | **CRADLE 발사(dry)** | `G`→`SPACE` | RELAY1 ON 펄스, 솔레노이드 딸깍(그물·CO2 없이) |
| 6b | **TRAWLER 발사(dry)** | `M`→`G`→`SPACE` | RELAY2 ON 펄스, 대형 솔레노이드 딸깍 |
| 6c | **cord-cut(dry)** | (TRAWLER,ARM) `X` | RELAY3 짧은 펄스, 니크롬 적열(가연물 치우고, ≤2s) |
...
1. **SITL/HIL 리허설**: `sim_vehicle.py -v ArduCopter` ↔ 콕핏으로 두 미션 흐름 —
   CRADLE(조준→아밍→발사→시닝→윈치→RTL)과 TRAWLER(`M`→표준오프 조준→발사→cord-cut→RTL) — 손에 익히기.
2. **테더드 호버**: 마당에서 끈 묶고 저고도 호버 30초 → 진동(VIBE)/온도/전류 로그 확인. **링크 끊어보기**(페일세이프 RTL/LAND).
3. **그물 야외 발사시험(무인지대)**: 풍선/저가 테스트 드론 표적.
   - **CRADLE**: `IJKL` 조준 → `G`→`SPACE` → **전개·포획·시닝** 실측.
   - **TRAWLER**: `M`으로 전환 → **안전거리에서** 발사 → 대형 그물 전개·결속 확인 → `X`로 **cord-cut 분리** 실측(테더 절단·투기). 안 잡히면 조준 오프셋/발사 타이밍/패킹·추 대칭 튜닝.
...
| TRAWLER 발사 안 됨 | RELAY2/카트리지 | dry로 RELAY2부터, 33g 카트리지·아밍 확인 |
| cord-cut 안 끊김/계속 적열 | 소선 접촉/타임아웃 | 소선이 테더 감쌌는지, ON 타임아웃≤2s, 단선 점검 |
...
- [ ] BOM 77라인 조달/외주/차고제작 완료(완성품 2개).
- [ ] 드론: 추력 ≥2:1, MTOW 14kg 내, 전원/퓨즈 docs/06대로, 페일세이프 RTL/LAND 검증.
- [ ] **페이로드(CRADLE): 조준(SERVO9/10)·시닝(11)·릴리스(12)·발사(RELAY0) 전부 콕핏에서 실동작.**
- [ ] **페이로드(TRAWLER): 대형 발사(RELAY1)·cord-cutter(RELAY2, ON 타임아웃≤2s) 콕핏에서 실동작.**
